...
export default function Unauthorized() {
    ...
    const status = searchParams.get('status') || location.state?.status || '403'
        ...
        <div className="flex min-h-screen items-center justify-center bg-[#EEF4F2] px-6 py-16 text-slate-900 font-sans">
            <div className="w-full max-w-2xl rounded-[2.5rem] border border-slate-100 bg-white p-12 text-center shadow-sm">
                <div className="space-y-6">
                    {is401 ? (
                        <>
                            <p className="text-sm font-bold tracking-widest text-[#00695C] uppercase">
                                {t('errors.unauthenticatedTitle', 'تحتاج لتسجيل الدخول')}
                            </p>
                            <h1 className="text-7xl sm:text-8xl font-black text-slate-200">401</h1>
                            <h2 className="text-2xl sm:text-3xl font-extrabold text-slate-800">
                                {t('errors.unauthenticatedMsg', 'عذراً، يرجى تسجيل الدخول أولاً للوصول إلى هذه الصفحة')}
                            </h2>
                            <p className="max-w-md mx-auto text-base sm:text-lg leading-relaxed text-slate-500 font-medium pb-4">
                                {t('errors.unauthenticatedDesc', 'الرمز المميز (Token) غير متوفر أو انتهت صلاحيته. يرجى تسجيل الدخول للتعرف على هويتك.')}
                            </p>
                        ...
                        <>
                            <p className="text-sm font-bold tracking-widest text-red-600 uppercase">
                                {t('errors.unauthorizedTitle', 'غير مصرح')}
                            </p>
