            ...
            <div className="space-y-4 sm:space-y-6">
                <div className="grid gap-4 sm:gap-6 grid-cols-1 lg:grid-cols-2">
                    <div className="rounded-2xl sm:rounded-3xl border border-slate-200/80 bg-white/90 p-4 sm:p-6 shadow-soft backdrop-blur-xl dark:border-slate-700/60 dark:bg-slate-900/80">
                        <h2 className="text-lg sm:text-xl font-semibold text-slate-900 dark:text-slate-100">{t('admin.performanceTitle')}</h2>
                        <p className="mt-2 sm:mt-3 text-sm text-slate-500 dark:text-slate-400">{t('admin.performanceDescription')}</p>
                        <div className="mt-4 sm:mt-6 flex flex-wrap gap-2 sm:gap-3">
                            {['dashboard.kpi.conversion', 'dashboard.kpi.engagement', 'dashboard.kpi.impact'].map((item) => (
                                <span key={item} className="rounded-full bg-slate-100 px-3 sm:px-4 py-1.5 sm:py-2 text-xs sm:text-sm text-slate-700 dark:bg-slate-800 dark:text-slate-200">{t(item)}</span>
                            ))}
                        </div>
                    </div>
                    <div className="rounded-2xl sm:rounded-3xl border border-slate-200/80 bg-white/90 p-4 sm:p-6 shadow-soft backdrop-blur-xl dark:border-slate-700/60 dark:bg-slate-900/80">
                        <h2 className="text-lg sm:text-xl font-semibold text-slate-900 dark:text-slate-100">{t('admin.activityTitle')}</h2>
                        <p className="mt-2 sm:mt-3 text-sm text-slate-500 dark:text-slate-400">{t('admin.activityDescription')}</p>
                        <ul className="mt-4 sm:mt-5 space-y-2 sm:space-y-3 text-sm text-slate-600 dark:text-slate-300">
                ...
                <div>
                    <h2 className="mb-3 sm:mb-4 text-lg sm:text-xl font-semibold text-slate-900 dark:text-slate-100">{t('admin.userDirectoryTitle')}</h2>
